...
//
// Fully custom — no Supabase auth dependency (bypasses free-tier email limits)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find the latest valid, unverified OTP session for this email
    const { data: session, error: sessionErr } = await supabase
      .from("otp_sessions")
      .select("id, otp_hash, attempt_count")
      .eq("mobile", email)       // mobile col stores identifier (email here)
